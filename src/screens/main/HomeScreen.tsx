...
const HomeScreen = () => {
  ...
  const { footprint, loading, error } = useSelector((state: RootState) => state.carbon);
  const { profile } = useSelector((state: RootState) => state.user);

  const handleAddTransportation = () => {
    dispatch(updateFootprint({ transportation: footprint.transportation + 2.5 }));
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.categoriesContainer}>
          ...
          <TouchableOpacity style={styles.categoryButton} onPress={handleAddTransportation}>
            <Text style={styles.categoryEmoji}>🚗</Text>
            <Text style={styles.categoryText}>Drive 10km</Text>
            <Text style={styles.categoryValue}>+2.5 kg</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.categoryButton} onPress={handleAddFood}>
            <Text style={styles.categoryEmoji}>🍖</Text>
            <Text style={styles.categoryText}>Meat meal</Text>
            <Text style={styles.categoryValue}>+1.2 kg</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.categoryButton} onPress={handleAddEnergy}>
            <Text style={styles.categoryEmoji}>💡</Text>
            <Text style={styles.categoryText}>1 hour AC</Text>
            <Text style={styles.categoryValue}>+3.1 kg</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.categoryButton} onPress={handleAddWaste}>
        ...
        <View style={styles.breakdownContainer}>
          ...
          <View style={styles.breakdownItem}>
            ...
            <Text style={styles.breakdownValue}>{footprint.transportation.toFixed(1)} kg</Text>
          </View>
          <View style={styles.breakdownItem}>
            <Text style={styles.breakdownLabel}>🍽️ Food</Text>
            <Text style={styles.breakdownValue}>{footprint.food.toFixed(1)} kg</Text>
          </View>
          <View style={styles.breakdownItem}>
            <Text style={styles.breakdownLabel}>⚡ Energy</Text>
            <Text style={styles.breakdownValue}>{footprint.energy.toFixed(1)} kg</Text>
          </View>
          <View style={styles.breakdownItem}>
            <Text style={styles.breakdownLabel}>🗑️ Waste</Text>
            <Text style={styles.breakdownValue}>{footprint.waste.toFixed(1)} kg</Text>
          </View>
        </View>
