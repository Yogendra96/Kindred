...
class DeviceAttestationService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      modernAPMService.recordMetric('device_attestation_init', Date.now() - startTime, 'ms');
   ...
   */
  async performDeviceAttestation(forceRefresh = false): Promise<AttestationResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      modernAPMService.recordMetric('device_attestation_time', attestation.attestationTime, 'ms');
   ...
   */
  async verifyDeviceIdentity(expectedIdentity: DeviceIdentity): Promise<boolean> {
    try {
   ...
   */
  recordSuspiciousActivity(activity: string, severity: 'low' | 'medium' | 'high'): void {
