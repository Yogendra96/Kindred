...
export class CarbonCalculatorCore {
  ...
  async batchCalculate(requests: CarbonCalculationRequest[]): Promise<CarbonCalculationResponse[]> {
    ...
    try {
      ...
      this.logger.error('Batch calculation failed', {
        batchSize: requests.length,
        error: error instanceof Error ? error.message : 'Unknown error',
      }, error as Error);
      throw error;
  ...
  static convertToFormCalculation(response: CarbonCalculationResponse): CarbonEmissionCalculation {
    return {
      ...
      offset_cost_usd: Number((response.emissions * CARBON_CONVERSION.OFFSET_COST_PER_KG).toFixed(2)),
  ...
  private getEmissionFactor(
    ...
    switch (activityType) {
      case 'transport': {
        ...
        const factor = EMISSION_FACTORS.TRANSPORT[vehicleType as keyof typeof EMISSION_FACTORS.TRANSPORT];
      ...
      case 'energy': {
        ...
        const factor = EMISSION_FACTORS.ENERGY[energySource as keyof typeof EMISSION_FACTORS.ENERGY];
        if (!factor) return null;
