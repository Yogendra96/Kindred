...
export const SmartCameraCapture: React.FC<SmartCameraCaptureProps> = ({
  ...
  useEffect(() => {
    if (visible) {
      ...
      animateIn();
      // Ensure AI Vision is initialized
      aiVisionService.initialize().catch(err => {
        console.error('Failed to initialize AI Vision', err);
        setState(prev => ({ ...prev, error: 'Failed to initialize AI' }));
      });
    }
  }, [visible]);
