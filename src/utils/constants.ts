...
export const FUEL_TYPES = [
  { id: 'gasoline', name: 'Gasoline', factor: EMISSION_FACTORS.TRANSPORT.CAR_GASOLINE },
  { id: 'diesel', name: 'Diesel', factor: EMISSION_FACTORS.TRANSPORT.CAR_DIESEL },
  { id: 'electric', name: 'Electric', factor: EMISSION_FACTORS.TRANSPORT.CAR_ELECTRIC },
  { id: 'hybrid', name: 'Hybrid', factor: EMISSION_FACTORS.TRANSPORT.CAR_GASOLINE * 0.7 },
  { id: 'plugin-hybrid', name: 'Plug-in Hybrid', factor: EMISSION_FACTORS.TRANSPORT.CAR_GASOLINE * 0.5 },
  { id: 'natural-gas', name: 'Natural Gas', factor: EMISSION_FACTORS.ENERGY.NATURAL_GAS },
] as const;
...
export const getEmissionFactor = (category: string, type: string): number => {
  switch (category) {
    case 'transport':
      return EMISSION_FACTORS.TRANSPORT[type as keyof typeof EMISSION_FACTORS.TRANSPORT] ?? 0;
    case 'energy':
      return EMISSION_FACTORS.ENERGY[type as keyof typeof EMISSION_FACTORS.ENERGY] ?? 0;
    case 'food':
      return EMISSION_FACTORS.FOOD[type as keyof typeof EMISSION_FACTORS.FOOD] ?? 0;
    default:
      return 0;
  }
};
